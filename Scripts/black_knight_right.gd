...
@onready var current_location =  get_node("../../../Chessboard/B8")
...
func _ready() -> void:
	...
	$"Black Knight Right Mesh".material_override = mat
...
func select() -> void:
	var mesh_inst := $"Black Knight Right Mesh" as MeshInstance3D
	if not mesh_inst:
		return
