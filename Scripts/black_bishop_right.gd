...
@export var piece_type: String = "bishop"
@export var color: String = "black"
@onready var current_location =  get_node("../../../Chessboard/C8")
...
func _ready() -> void:
	...
	$"Black Bishop Right Mesh".material_override = mat
	...
	for dir in dirs:
		...
		while true:
			...
			if not Helper._is_within_board(c):
				break
			var sq = Helper.coords_to_square_name(c)
			var p = Helper._get_piece_at_square(sq)
			if p:
				if p.color != color:
					moves.append(sq)
				break
...
func select() -> void:
	var mesh_inst := $"Black Bishop Right Mesh" as MeshInstance3D
	if not mesh_inst:
		return
